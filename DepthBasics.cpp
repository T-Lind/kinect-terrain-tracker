...
void CDepthBasics::Update()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            // ASSIGN THE POINTER TO THE DEPTH BUFFER TO pBuffer!!! -TL
...
void CDepthBasics::ProcessDepth(INT64 nTime, const UINT16* pBuffer, int nWidth, int nHeight, USHORT nMinDepth, USHORT nMaxDepth)
{
    ...
    if (m_pDepthRGBX && pBuffer && (nWidth == cDepthWidth) && (nHeight == cDepthHeight))
    {
        ...
        while (pBuffer < pBufferEnd)
        {
            ...
            // Reset pixel values! Otherwise you get a ghosting effect
            pRGBX->rgbRed   = 0;
            pRGBX->rgbGreen = 0;
            pRGBX->rgbBlue  = 0;

            program.UpdateDepth(index, depth, pRGBX);
